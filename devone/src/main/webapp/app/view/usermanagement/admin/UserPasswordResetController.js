Ext.define('Devone.view.usermanagement.admin.UserPasswordResetController', {
			...
			success : function(response, currentObject, options) {
				...
				for (var i = 0; i < data.length; i++) {
					var obj = {
						...
						isLocked : data[i].userDetail.isLocked
					}
					tempData.push(obj);
				}
	...
	isSelectedUsersArentLocked : function(userGrid) {
		var isSelectionArntLocked = true;
		var rows = userGrid.getSelectionModel().getSelection();
		for (var x = 0; x < rows.length; x++) {
			if (rows[x].data.isLocked == true) {
				Ext.Msg.alert({
					title : 'Warning',
					msg : rows[x].data.firstName + " " + rows[x].data.lastName + ' user is locked, Please unlock the user to perform operations',
					icon : Ext.MessageBox.WARNING
				});

				isSelectionArntLocked = false;
				break;
			}
		}
		return isSelectionArntLocked;
	},

	//Method added here
	checkUserStatus : function(userGrid) {

		var usersJSON = [];
		var rows = userGrid.getSelectionModel().getSelection();
		for (var x = 0; x < rows.length; x++) {
			var obj = {
				userStatus : rows[x].data.isLocked,
			};
			usersJSON.push(obj);
		}
		return usersJSON;
	...
	onResetPwdClick : function(b) {
		...
		} else if (this.isSelectedUsersArentLocked(grid)) {
