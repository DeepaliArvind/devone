Ext.define('Devone.view.appreportui.querycriteria.QueryCriteriaViewController', {
	extend : 'Ext.app.ViewController',
	alias : 'controller.queryCriteriaController',
	requires : ['Devone.view.fw.DateRange','Devone.view.fw.DropDownRange','Devone.view.fw.MultiSelectDropDown'],
	...
	afterQCPanelAdded:function()
	{
		...
		//add only display field true in query criteria
						var finalQueryCriteria=[];
		for(var i=0;i<currentView.reportDataJson.queryCWidgetU.length;i++){
		  if(currentView.reportDataJson.queryCWidgetU[i].isDisplay==undefined){
		  finalQueryCriteria.push(currentView.reportDataJson.queryCWidgetU[i]);
		  }
		}
		qcInnerPanel.add(finalQueryCriteria);
		...
		//if any other value pass then override default value to new search values
			...
			for (var x = 0; x < queryCriteria.length; x++) {
				...
				for (var x1 = 0; x1 < hrefqc.length; x1++) {
					if (hrefqc[x1].name == queryCriteria[x].name) {
						match = true;
						/*if(queryCriteria[x].datatypecheck!=undefined && queryCriteria[x].datatypecheck!=null && queryCriteria[x].datatypecheck=="dropdownrange"){
							hrefqc[x1].value=queryCriteria[x].value;
						}*/
						break;
					}
				}
		...
		Ext.Array.each(this.getView().down('#qcInnerPanelId').items.items, function(item, idx, items) {
			...
			if (item.xtype == "adatepickerfield" || item.xtype == 'datefield') {
				...
				var defaultJson = {
					...
					label : item.getFieldLabel(),
					fromui : true,
					comboValue:item.xtype=="combobox" ? item.lastMutatedValue:null,
					datatype : item.datatype,
					value : val,
					index : item.index
				};
	...
	loadDataInWidget : function(widget) {
		...
		Ext.Ajax.request({
			...
			method :widget.requestMethodType,
			async:false,
			scope : me,
			widget:widget,
			jsonData :{},
	...
	filterData : function(btn) {
		...
		if(reportVIEW.reportJSON.data_json.displayType=="0"){
		...
		store.getProxy().extraParams = reportVIEW.controller.getGridParams(this.getView());
		
		reportVIEW.controller.getDataPointChartDataByAjax(this.getView());
		store.load();
		
		try{
	    //close query criteria window  on evey search click if report view is panel 
		var queryCriteripanel=btn.up("queryCriteriaNorthPanel");
		if(queryCriteripanel!=undefined || queryCriteripanel!=null){
				queryCriteripanel.up().close();
			}
		}catch(err){
		}
	},
