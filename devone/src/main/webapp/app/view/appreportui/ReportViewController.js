Ext.define('Devone.view.appreportui.ReportViewController', {
	...
	createCenterPanelViewComponents:function()
	{
			...
			northPanel.add({
				tools : [{
					...
					handler : function(btn, eOpts) {
						...
						window.add(Ext.create('Devone.view.appreportui.querycriteria.QueryCriteriaPanelView',{
		...
		if(this.reportView.reportJSON.data_json.isGridDisplay!=undefined && this.reportView.reportJSON.data_json.isGridDisplay=="1"){
	...
	{
	if(this.reportView.reportJSON.data_json.queryCWidgetU.length>=1){
	var finalArrayOfQC=[];
	Ext.Array.each(this.reportView.reportJSON.data_json.queryCWidgetU, function(item, idx, items) {
			var val;
			if(item.sessionInput!=true){
				...
				var defaultJson = {
					...
					margin:'0 15 0 0',
					labelAlign:'left',
					fieldLabel : "<b>"+item.fieldLabel+"</b>",
					value:item.defaultValue
				};
				finalArrayOfQC.push(defaultJson);
			} else if (item.xtype == "daterange") {
				finalArrayOfQC.push({
					name : item.fromname,
					xtype: 'displayfield',
					itemId:item.fromname,
					margin:'0 15 0 0',
					labelAlign:'left',
				...
				finalArrayOfQC.push({
					name : item.fromName,
					 ...
					 margin:'0 15 0 0',
					 labelAlign:'left',
					fieldLabel : "<b>"+item.fromFieldLabel+"</b>"
				});
				finalArrayOfQC.push({
					name : item.toName,
					 xtype: 'displayfield',
					 ...
					 margin:'0 15 0 0',
					 labelAlign:'left',
					fieldLabel : "<b>"+item.toFieldLabel+"</b>"
				});
	...
	//Take Query Criteria for setting to tool above Grid		
		var headerQC=this.reportView.down('#querycriteria')!=null? this.reportView.down('#querycriteria').controller.fetchQueryCriteria():[];
		if(headerQC.length>=1){		
				var gridViewHeader=this.getView().down("dataGridView").getHeader().items.items;
				for(var i=0;i<gridViewHeader.length;i++){
					var upperItem =gridViewHeader[i];
				 	for(var j=0;j<headerQC.length;j++){
				 		var item1=headerQC[j];
				 		if(upperItem.name===item1.name){
				 		var itemIdName="#"+item1.name;
				 		var itemValue=item1.value;
				 		if(item1.datatype=="DATE" || item1.datatype=="TIMESTAMP"){
				 			var date=new Date(item1.value);
					 		this.getView().down("dataGridView").down(itemIdName).setValue(Ext.Date.format(date,'d-m-y'));
				 		}else{
				 			if(itemValue=="%"){
				 			itemValue="All";
				 			}
				 			if(item1.comboValue!=undefined && item1.comboValue!=null){
				 			itemValue=item1.comboValue;
				 			}
				 			this.getView().down("dataGridView").down(itemIdName).setValue(itemValue);
				 		}
