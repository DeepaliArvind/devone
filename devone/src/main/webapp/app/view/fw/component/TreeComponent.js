Ext.define('Devone.view.fw.component.TreeComponent', {
			...
			success : function(response,obj){
				...
				if(levelInfo.length>0){
					...
					if(Array.isArray(data)){
						...
						nodes = data[firstLevel.innerCollection[0]];
					}
										
					if(nodes && nodes.length>0){
						scope.generateTreeNodes(nodes,firstLevel.innerNodes[0]);
	...
	generateTreeNodes :function(data,nodeLevel){
        ...
        if(data){
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if(nodeLevel && (!info.hasOwnProperty("text")&&!info.hasOwnProperty("children"))){
                    ...
                    if(innerNodes.length>0){
                        ...
                        for (var j = 0; j < innerNodes.length; j++) {
                            ...
                            var idx = nodeLevel.innerCollection.hasOwnProperty(j)?j:0;

                          /*  var arrayNode = {};
                            arrayNode["text"] = nodeLevel.innerCollection[idx];
                            arrayNode["children"] = [];
                            	arrayNode["leaf"] = false;
                            	  arrayNode["expanded"] =  true;*/

                            var innData = this.generateTreeNodes(info[nodeLevel.innerCollection[idx]],innerNode);
                         //   arrayNode.children = arrayNode.children.concat(innData);
