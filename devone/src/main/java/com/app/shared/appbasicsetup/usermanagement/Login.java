...
@NamedQueries({ @javax.persistence.NamedQuery(name = "Login.findAll", query = " select u from Login u where u.systemInfo.activeStatus=1"), @javax.persistence.NamedQuery(name = "Login.DefaultFinders", query = "select e from Login e where e.systemInfo.activeStatus=1 and e.serverAuthImage LIKE :serverAuthImage"), @javax.persistence.NamedQuery(name = "Login.findByContactId", query = "select e from Login e where e.systemInfo.activeStatus=1 and e.coreContacts.contactId=:contactId"), @javax.persistence.NamedQuery(name = "Login.findByUserId", query = "select e from Login e where e.systemInfo.activeStatus=1 and e.userDetail.userId=:userId"), @javax.persistence.NamedQuery(name = "Login.findById", query = "select e from Login e where e.systemInfo.activeStatus=1 and e.loginPk =:loginPk"), @javax.persistence.NamedQuery(name = "Login.FindMappedUser", query = "SELECT e FROM Login e WHERE e.userDetail.userId IN (SELECT ub.userId FROM UserRoleBridge ub) AND e.systemInfo.activeStatus=1"), @javax.persistence.NamedQuery(name = "Login.FindUnMappedUser", query = "SELECT e FROM Login e WHERE e.userDetail.userId NOT IN (SELECT ub.userId FROM UserRoleBridge ub) AND e.systemInfo.activeStatus=1") })
public class Login implements Serializable, CommonEntityInterface, UserAuthentication, Comparator<Login> {

    private static final long serialVersionUID = 1483948524877L;
